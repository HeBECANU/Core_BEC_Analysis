function [ zfilt ] = gaussfiltn(nt,z,nsigma)
if ismatrix(nt)
    zfilt = gaussfilt(nt,z,nsigma);
else
%Apply a Gaussian filter to a n-dim time series
...
zfilt = z;
for d = 1:size(nt,1) %run through each dimension
    t = nt{d};
    sigma = nsigma(d);
n = size(z,d);  % number of data
...
if uniform
    ...
    filter(i) = [];
    zfilt = convn(zfilt,filter,'same');
    onesToFilt = ones(size(z));     % remove edge effect from conv 
    onesFilt = convn(onesToFilt,filter,'same'); 
